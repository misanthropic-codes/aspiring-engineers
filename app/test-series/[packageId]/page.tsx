...
export default function PackageDetailPage() {
  ...
  useEffect(() => {
    const fetchPackage = async () => {
      ...
      try {
        const response = await apiClient.get<PackageDetailResponse>(`/packages/${packageId}`);
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className={`p-6 rounded-2xl border ${
                ...
                {packageData.thumbnail ? (
                  ...
                  <div className={`w-full h-64 flex items-center justify-center rounded-xl mb-6 ${
                    darkMode ? 'bg-gradient-to-br from-[#2596be]/20 to-[#4EA8DE]/20' : 'bg-gradient-to-br from-[#2596be]/10 to-[#4EA8DE]/10'
                  }`}>
                    <BookOpen className={`w-24 h-24 ${darkMode ? 'text-[#2596be]' : 'text-[#2596be]/60'}`} />
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6 pt-6 border-t border-dashed border-gray-200/20">
                  <div className="text-center">
                    ...
                    <div className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Tests</div>
                  </div>
                  <div className="text-center">
                    <div className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                      {packageData.totalQuestions.toLocaleString()}
                    </div>
                    <div className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Questions</div>
                  </div>
                  <div className="text-center">
                    <div className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                      {packageData.validityDays}
                    </div>
                    <div className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Days Validity</div>
                  </div>
                  <div className="text-center">
                    <div className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                      {packageData.metadata?.totalStudents?.toLocaleString() || 0}
                    </div>
                    <div className={`text-sm ${darkMode ? 'text-gray-500' : 'text-gray-500'}`}>Students</div>
                  </div>
                </div>
              </div>
                      ...
                      >
                        <div className="flex items-center gap-4">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                            ...
                            <span className="text-[#2596be] font-semibold">{idx + 1}</span>
                          </div>
                          <div>
            ...
            <div className="lg:col-span-1">
              <div className={`sticky top-24 p-6 rounded-2xl border ${
                ...
                <div className="space-y-4 pt-6 border-t border-dashed border-gray-200/20">
                  {packageData.metadata?.rating && (
                    <div className="flex items-center justify-between">
                      <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Rating</span>
                  ...
                  {packageData.metadata?.language && (
                    <div className="flex items-center justify-between">
                      <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Language</span>
                      <span className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {packageData.metadata.language}
                      </span>
                    </div>
                  ...
                  {packageData.metadata?.targetYear && (
                    <div className="flex items-center justify-between">
                      <span className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Target Year</span>
                      <span className={`font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {packageData.metadata.targetYear}
                      </span>
                    </div>
                ...
                {/* Instructors */}
                {packageData.metadata?.instructors && packageData.metadata.instructors.length > 0 && (
                  <div className="pt-6 mt-6 border-t border-dashed border-gray-200/20">
                    ...
                    <div className="space-y-2">
                      {packageData.metadata.instructors.map((instructor, idx) => (
                        <div key={idx} className="flex items-center gap-2">
                          <GraduationCap className={`w-4 h-4 ${darkMode ? 'text-gray-500' : 'text-gray-400'}`} />
